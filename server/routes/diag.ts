...
export const handleApiDiagnostics: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const diagnostics = {
      ...
      tests: {
        health: {
          ...
          isValidJson:
            healthResult.success && healthResult.body
              ? (() => {
                  try {
        ...
        routes: {
          ...
          isValidJson:
            routesResult.success && routesResult.body
              ? (() => {
                  try {
                    ...
                    return {
                      ...
                      scraperStartExists:
                        parsed.critical_check?.scraper_start || false,
                      data: parsed,
                    };
                  } catch {
                    return { valid: false };
        ...
        scraperStart: {
          ...
          isValidJson:
            scraperResult.success && scraperResult.body
              ? (() => {
                  try {
    ...
    console.log(
      `[DIAG] Diagnostics completed. Overall OK: ${summary.overallOk}`,
    );
...
function generateRecommendations(summary: any, diagnostics: any): string[] {
  ...
  if (!summary.dnsOk) {
    recommendations.push(
      "❌ DNS resolution failed. Check if glow-nest-api.fly.dev domain exists and is accessible.",
    );
  }
  ...
  if (!summary.healthOk) {
    if (diagnostics.tests.health.success) {
      if (diagnostics.tests.health.status !== 200) {
        recommendations.push(
          `❌ Health endpoint returned ${diagnostics.tests.health.status}. API might be down or misconfigured.`,
        );
      } else if (!diagnostics.tests.health.isValidJson.valid) {
        recommendations.push(
          "❌ Health endpoint returned non-JSON response. This might not be FastAPI.",
        );
      } else if (!diagnostics.tests.health.isValidJson.hasOk) {
        recommendations.push(
          "❌ Health endpoint JSON missing 'ok' field. Wrong API implementation.",
        );
      }
    } else {
      recommendations.push(
        "❌ Health endpoint unreachable. API server is down or URL is wrong.",
      );
    }
  }
  ...
  if (!summary.routesOk) {
    recommendations.push(
      "❌ Debug routes endpoint failed. FastAPI debug endpoint missing or broken.",
    );
  }
  ...
  if (!summary.scraperOk) {
    if (diagnostics.tests.scraperStart.success) {
      if (diagnostics.tests.scraperStart.status === 404) {
        recommendations.push(
          "❌ Scraper start endpoint not found (404). Route not implemented.",
        );
      } else if (diagnostics.tests.scraperStart.status >= 500) {
        recommendations.push(
          "❌ Scraper start endpoint server error. Check FastAPI logs.",
        );
      }
    } else {
      recommendations.push(
        "❌ Scraper start endpoint unreachable. Same as health endpoint issue.",
      );
    }
  }
  ...
  if (summary.overallOk) {
    recommendations.push(
      "✅ All diagnostics passed! API is working correctly.",
    );
  } else {
    recommendations.push(
      "🔧 Run: fly deploy -c fly.api.toml -a glow-nest-api to deploy the API if it's missing.",
    );
  }
