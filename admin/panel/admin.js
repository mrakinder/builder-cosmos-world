...
function updateSystemOverview(status) {
    ...
    const activeModules = Object.values(status)
        .filter(module => module && module.status === 'running').length;
...
function updateModuleStatuses(status) {
    ...
    updateModuleStatus('scraper', scraperStatus.status || 'idle', 
                      scraperStatus.progress || 0);
    
    // ML status
    const mlStatus = status.ml || {};
    updateModuleStatus('ml', mlStatus.status || 'idle',
                      mlStatus.progress || 0);
    
    // Prophet status
    const prophetStatus = status.prophet || {};
    updateModuleStatus('prophet', prophetStatus.status || 'idle',
...
function addLogEntry(eventData) {
    ...
    logEntry.textContent = 
        `[${timestamp}] [${eventData.module?.toUpperCase()}] ${eventData.details}`;
...
async function startScraping(listingType) {
    try {
        ...
        if (result.success) {
            showAlert(`✅ Скрапінг ${listingType} запущено! ${result.message}`, 'success');
...
async function trainMLModel() {
    try {
        ...
        if (result.success) {
            showAlert(`🏋️ Навчання моделі запущено! Ціль: MAPE ≤ ${result.target_mape}%`, 'success');
    ...
    mlProgressSource.onmessage = function(event) {
        try {
            ...
            if (progress.status === 'completed') {
                showAlert(`✅ Навчання завершено! MAPE: ${progress.final_mape}%`, 'success');
                mlProgressSource.close();
...
async function testMLPrediction() {
    try {
        ...
        if (result.success) {
            showAlert(`🔮 Тестовий прогноз: $${result.predicted_price} (65м², Центр, 2 кімнати)`, 'info');
        } else {
...
async function generateForecasts() {
    try {
        ...
        if (result.success) {
            showAlert(`📈 Прогнозування запущено на ${result.forecast_months} місяців`, 'success');
...
async function viewStreetMappings() {
    try {
        ...
        if (count > 0) {
            const sample = Object.entries(mappings).slice(0, 3)
