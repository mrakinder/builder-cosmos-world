...
export default function Index() {
  ...
  const streetToDistrict: { [key: string]: string[] } = {
    ...
    "Вербицького": ["Брати"],
    "Вовчинецька": ["Позитрон", "Кішлак", "Софіївка", "Залізнични�� (Вокзал)"],
    "Галицька": ["Пасічна"],
    ...
    "Львівська": ["Центр"],
    "Мазепи": ["Хриплин"],
    "Марійки Підгірянки": ["Залізничний (Вокз��л)"],
    "Матейка": ["Центр"],
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (response.ok) {
                            alert(`✅ ${data.message}\nОчікува��ий час: ${data.estimatedTime}`);
                          } else {
