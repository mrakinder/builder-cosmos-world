...
import { API_CONFIG, getHealthUrl, getDebugRoutesUrl, getScraperStartUrl } from './shared/config';
...
export async function runClientSideTest(): Promise<AdminTestResult> {
  try {
    ...
    const scraperOk = scraperResult.ok && (scraperResult.status === 202 || scraperResult.status === 409);
 ...
 */
export async function runAdminPanelTests(addLogEntry: (message: string) => void): Promise<boolean> {
  addLogEntry('🔧 ADMIN SELF-TEST STARTED');
  addLogEntry(`🎯 Target API: ${API_CONFIG.BASE_URL}`);
  addLogEntry(`🚫 Localhost banned: ${API_CONFIG.BASE_URL.includes('localhost') ? '❌ DETECTED' : '✅ Confirmed'}`);
  ...
  if (serverResult.ok && serverResult.data) {
    ...
    addLogEntry(`   DNS: ${summary?.dnsOk ? '✅' : '❌'} | Health: ${summary?.healthOk ? '✅' : '❌'} | Routes: ${summary?.routesOk ? '✅' : '❌'} | Scraper: ${summary?.scraperOk ? '✅' : '❌'}`);
  }
  ...
  if (clientResult.ok && clientResult.data) {
    addLogEntry(`   Health: ${clientResult.data.health?.ok ? '✅' : '❌'} | Routes: ${clientResult.data.routes || 0} total | Scraper: ${clientResult.data.scraper ? '✅' : '❌'}`);
  }
