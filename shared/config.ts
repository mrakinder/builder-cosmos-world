...
export const getDebugRoutesUrl = (): string =>
  buildApiUrl(API_CONFIG.ENDPOINTS.DEBUG_ROUTES);
export const getScraperStartUrl = (): string =>
  buildApiUrl(API_CONFIG.ENDPOINTS.SCRAPER_START);
export const getProgressStreamUrl = (): string =>
  buildApiUrl(API_CONFIG.ENDPOINTS.PROGRESS_STREAM);
export const getEventsStreamUrl = (): string =>
  buildApiUrl(API_CONFIG.ENDPOINTS.EVENTS_STREAM);
...
export const logApiRequest = (
  ...
  if (body) {
    console.log(
      `[API-REQUEST] Body: ${JSON.stringify(body).substring(0, 200)}${JSON.stringify(body).length > 200 ? "..." : ""}`,
    );
  }
};
...
export const logApiResponse = (
  ...
  if (body) {
    console.log(
      `[API-RESPONSE] Body: ${body.substring(0, 200)}${body.length > 200 ? "..." : ""}`,
    );
  }
};
